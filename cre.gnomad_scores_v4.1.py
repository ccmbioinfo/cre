...
constraints=pd.read_csv("/hpf/largeprojects/ccmbio/ajain/crg2_hg38/gnomad_v4.1/constraint_metrics/gnomad.v4.1.constraint_metrics.tsv",sep="\t")

#Filtering for the Ensembl canonical transcript
constraints=constraints[constraints["gene_id"].str.startswith("ENSG") & constraints["mane_select"]==True]

#Check if there are any duplicates
if len(constraints["gene_id"]) == len(set(constraints["gene_id"])):
    print(f"There are no duplicates in this dataframe. There are {len(constraints['gene_id'])} unique ensembl mane select genes")
else:
    print("Duplicates found!")

# Keeping columns that are relevant
constraints=constraints[["gene_id",
                         "transcript","lof.oe","lof.oe_ci.lower","lof.oe_ci.upper",
             "lof.pLI","lof.pNull","lof.pRec","mis.oe","mis.z_score"]]
