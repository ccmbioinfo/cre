...
create_report <- function(family, samples, type){
  ...
  for (i in 1:nrow(variants)){
    ...
    if (coding){
      gene_impacts <- subset(impacts, (variant_id == v_id) & (is_coding == 1 | aa_change == "splice_region_variant"),
  ...
  # replace -1 with 0
  for (field in c("Gnomad_af", "Gnomad_af_grpmax")){
    variants[,field] <- with(variants, gsub("-1", "0", get(field), fixed = T))
    variants[,field] <- with(variants, gsub("None", "0", get(field), fixed = T))
  }
...
select_and_write2 <- function(variants, samples, prefix, type){
  ...
  variants_column_check = c(c("Position", "UCSC_Link", "GNOMAD_Link", "Ref", "Alt"),
                            c("Gene"),
                            ...
                            "CMP_samples", "CMP_samples_counts")
  
  print("CHECK! Print columns not in report")
  print(setdiff(variants_column_check, colnames(variants)) )
...
clean_and_output_report <- function(out){
  ...
  column_order = c('Gene','Position','UCSC_Link','GNOMAD_Link','Ref','Alt','gts',
                   ...
                   'Vest4_score','Revel_score','Gerp_score', 'AlphaMissense', 
                   'ncER_score', 'ReMM_score', 'LINSIGHT_score', 
                   'Imprinting_status','Imprinting_expressed_allele','Pseudoautosomal',
                   'Old_multiallelic','UCE_100bp','UCE_200bp','DNaseI_hypersensitive_site','CTCF_binding_site',
                   'ENH_cellline_tissue','TF_binding_sites',"GreenDB_variant_type", "GreenDB_closest_gene", "GreenDB_controlled_gene",
                   "CMP_samples", "CMP_samples_counts")
  
  print("CHECK! Print columns not in final output")
  print(setdiff(column_order, colnames(out)) )
  
  out = out[,column_order]
  
  # Report generation for CMP analysis
  # 1. intronic variants of high interest 
  library(dplyr)
  intronic_report = out[grepl("intron_variant|upstream_gene_variant|downstream_gene_variant|intergenic_variant", out$Variation), ]
  intronic_report$on = paste0(intronic_report$Position, intronic_report$Ref, ">", intronic_report$Alt)
  Patho = intronic_report[grepl("Pathogenic", intronic_report$Clinvar),]
  nrow(Patho)
  Splice = intronic_report[intronic_report$SpliceAI_score > 0.8 & !(grepl("Benign|benign",intronic_report$Clinvar)) , ]
  nrow(Splice)
  intronic_report = intronic_report[intronic_report$on %in% c(Patho$on, Splice$on),]
  intronic_report$on = NULL
  write.csv(intronic_report, paste0(family,".", type, ".", datetime,".intronic-filter.csv"), row.names = F)
  
  # 2. Other variants of interest - coding and non-intronic
  out = out[!(out$Variation %in% c("None", "intron_variant", "upstream_gene_variant", "downstream_gene_variant", "intergenic_variant")),]
  write.csv(out, paste0(family,".", type, ".", datetime,".soft-filtered.csv"), row.names = F)
